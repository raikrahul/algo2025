...
* **Why it failed:** `slow` does not own the memory. `slow` cannot give the memory to `head`. [cite_start]To fix this, you would need to `clone` (photocopy the gold bars), which violates the "Efficient / Single Pass" rule[cite: 45].
